!Program to solve a two-component Fisher equation in two dimensions, with noise
!Equations are is dphi_i/dt = alpha phi_i (1-phi_1-phi_2) + D grad^2 phi_i + sqrt(phi_i)*noise i=1,2
...
contains
  	...
  	subroutine shift_everything() !shift all the fields so that the contour is roughly centred in the sim box
 		...
 		real(8) :: tolerance
        integer, dimension(SIZE,SIZE) :: temp_n, temp_m
 		...
 		do j = 1,SIZE
 			do i=1,SIZE
 				if(n(i,j)+psi_1(i,j)<tolerance/phi_min .and.  &
                m(i,j) + psi_2(i,j)<tolerance/phi_min .and. j<ymin) then 
 					ymin=j
 				endif
 			enddo
 		enddo
	...
	subroutine print_grid(label)
		...
		write(filename,"(A4,I0,A4)") '2out',label, '.dat'
